...
function Row({ heading, tab, url }) {
  ...
  // Determine route type based on heading and displayedIndex
  const getRouteType = () => {
    if (heading === "Trending") {
      return "movie"; // Both Day and Week are movies
    }
    if (heading === "What's Popular") {
      return displayedIndex === 0 ? "movie" : "tv";
    }
    if (heading === "Top Rated") {
      return displayedIndex === 0 ? "tv" : "movie";
    }
    ...
    <section className="px-4 sm:px-6 lg:px-14 py-12" style={{ backgroundColor: '#04152d' }}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <div
                          className={`backdrop-blur-sm rounded-full px-2 py-1 border ${
                            getRouteType() === "movie"
                        ...
                        >
                          <span
                            className={`text-xs font-semibold ${
                              getRouteType() === "movie" ? "text-green-400" : "text-blue-400"
                            }`}
                          >
                            {getRouteType() === "movie" ? "MOVIE" : "TV"}
                          </span>
                        </div>
      ...
      <div className="text-center mt-12">
        <div className="inline-flex items-center space-x-2 bg-white/5 backdrop-blur-sm rounded-full px-6 py-3 border border-white/10">
          <span className="text-amber-400 text-lg">{getRouteType() === "movie" ? "🎬" : "📺"}</span>
          <span className="text-gray-300 font-medium">
            Showing {Math.min(5, movies[displayedIndex]?.results?.length || 0)} of{" "}
            {movies[displayedIndex]?.results?.length || 0} {getRouteType() === "movie" ? "movies" : "TV shows"}
          </span>
